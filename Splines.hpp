...
namespace SL
{
    ...
    // Queries
    float Position0(const Spline1& spline);     // Starting position of spline
    float Position1(const Spline1& spline);     // End position of spline

    float Velocity0(const Spline1& spline);     // Starting (tangential) velocity
    ...
    int   SplinesFromPoints  (int numPoints, const Vec2f p[], int maxSplines, Spline2 splines[], float tension = 0.0f, size_t stride = sizeof(Vec2f));
    ...
    int   SplinesFromPointsDynamic(int numPoints, const Vec2f p[], int maxSplines, Spline2 splines[], float tension = 0.0f, float overshootRatio = 0.5f, size_t stride = sizeof(Vec2f));
    ...
    // Queries
    Vec2f Position0(const Spline2& spline); // Starting position of spline
    Vec2f Position1(const Spline2& spline); // End position of spline

    Vec2f Velocity0(const Spline2& spline); // Starting (tangential) velocity
    ...
    int   NumSplinesForPoints(int numPoints);     // Returns number of splines needed to represent the given number of points; generally n-1 except for n < 2.
    int   SplinesFromPoints  (int numPoints, const Vec3f p[], int maxSplines, Spline3 splines[], float tension = 0.0f, size_t stride = sizeof(Vec3f));
    // Fills 'splines' with splines that interpolate the points in 'p', and returns the number of these splines.
    // 'tension' controls the interpolation -- the default value of 0 specifies Catmull-Rom splines that
    // guarantee tangent continuity. With +1 you get straight lines, and -1 gives more of a circular appearance.

    int   SplinesFromPointsDynamic(int numPoints, const Vec3f p[], int maxSplines, Spline3 splines[], float tension = 0.0f, float overshootRatio = 0.5f, size_t stride = sizeof(Vec3f));
    // A version of SplinesFromPoints that adjusts tension per point to avoid overshoots/loops
    // caused by disparate segment sizes.
    ...
    // Queries
    Vec3f Position0(const Spline3& spline); // Starting position of spline
    Vec3f Position1(const Spline3& spline); // End position of spline

    Vec3f Velocity0(const Spline3& spline); // Starting (tangential) velocity
